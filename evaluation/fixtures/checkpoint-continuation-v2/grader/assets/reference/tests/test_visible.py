...
class ReleaseBuilderTests(unittest.TestCase):
    def test_records_are_validated_and_sorted(self):
        records = normalize_records(
            [
                {"id": "REL-2", "component": "worker", "status": "blocked", "notes": "waiting"},
                {"id": "REL-1", "component": "api", "status": "ready", "notes": "green"},
            ]
        )
    ...
    def test_prepare_round_trip(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            source.write_text(
                json.dumps([{"id": "REL-1", "component": "api", "status": "ready", "notes": "green"}]),
                encoding="utf-8",
            )
            prepare(source, target)
            self.assertEqual(load_checkpoint(target)["summary"], {"blocked": 0, "ready": 1})
