...
4. Confirm the successor received the capsule. Do not archive, pin, or otherwise hide either task.
5. Rename the source `<canonical title> - checkpoint <short source id>` and the successor `<canonical title>` so only the live task owns the canonical name.
...
If task tools are unavailable, return the capsule in chat and continue in place. Never claim that a successor exists unless its live task ID was confirmed.
