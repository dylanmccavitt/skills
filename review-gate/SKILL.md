...
Input: task contract, registered Review Gate actor, implementation receipt, and exact current head.

Allowed: inspect, test, report findings, and record the result through the locked state kernel.

Forbidden: write to the implementation branch, fix findings, share an actor identity with Implement, infer authority, or perform delivery.
